...
class Network(object):
    ...
    def connect(self, max_attempts=3):
        ...
        while attempts < max_attempts:
            ...
            if ip is None:
                logger.error('could not connect interface %s' %(self.iface.iface))
                self.disconnect()
                ...
                logger.info('got ip %s for interface %s' %(self.ip, self.iface.iface))
                break
        
        if attempts >= max_attempts:
            logger.error('could not connect to network after %s attempts' %(max_attempts))
            self.disconnect()
            return 1
...
class OpenvpnNetwork(Network):
    ...
    def connect(self):
        ...
        if vpn_ip is None:
            ...
            logger.info('got ip %s for interface %s' %(self.vpn_ip, self.vpn_iface))
    ...
    def disconnect(self):
        ...
        logger.info('disconnected from vpn network')
